...
import { result02 } from "./frontend02";

import { Bar } from "react-chartjs-2";
import { Chart } from "react-chartjs-2";

// import usersData from "./response.json";
import usersData from "./response02.json";
...
function QuizUsers() {
  ...
  const data = () => {
    const labels = [];
    result.forEach(
      ({ firstName, lastName, result: { html, css, js, react, redux } }) => {
        labels.push(
          `${firstName} ${lastName} ${html + css + js + react + redux}`
        );
      }
    );
  ...
  const data02 = () => {
    ...
    datasets.push({
      label: "HTML",
      data: result02.map(({ result: { html } }) => html),
      backgroundColor: "#f72585",
      stack: "Stack 0",
    });
    datasets.push({
      label: "CSS",
      data: result02.map(({ result: { css } }) => css),
      backgroundColor: "#7209b7",
      stack: "Stack 0",
    });
    datasets.push({
      label: "JS",
      data: result02.map(({ result: { js } }) => js),
      backgroundColor: "#3a0ca3",
      stack: "Stack 0",
    });
    datasets.push({
      label: "React",
      data: result02.map(({ result: { react } }) => react),
      backgroundColor: "#4cc9f0",
      stack: "Stack 0",
    });
    datasets.push({
      label: "Redux",
      data: result02.map(({ result: { redux } }) => redux),
      backgroundColor: "rgb(255, 99, 132)",
      stack: "Stack 0",
    });
  ...
  return (
    <>
      <AnswerWrapper>
        ...
        //   usersData
          users
        //   .filter(({firstName})=>firstName.startsWith("G2"))
          .map(({ id, firstName, lastName, answers }) => (
            <div key={id}>
              <h1>
      ...
      <Bar data={data02} options={options} />
    </>
  );
}
